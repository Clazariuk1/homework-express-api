...
// none of this is correct and I'm running in circles in how to even formulate the creation of a mock todo for correct testing. I'm under the impression I need to make the user BEFORE I make the todo but I don't have a firm grasp on this and it's choking me.
...
describe('Test suite for /todos route on api', () => {
    ...
    test('Should successfully create a todo post in the db', async () => {
        ...
        const todo = new Todo({ title: 'Create Todo', completed: false, user: 'Test User'})
    ...
    test('Should successfully update a given todo post', async () => {
        ...
        const response = await request(app)
        .put(`/todos/${todo._id}`)
        .send({ title: 'Updated Todo', completed: true, user: 'Namington Name' })

        expect(response.statusCode).toBe(200)
        expect(response.body.title).toEqual('Updated Todo')
        expect(response.body.user).toEqual('Namington Name')
